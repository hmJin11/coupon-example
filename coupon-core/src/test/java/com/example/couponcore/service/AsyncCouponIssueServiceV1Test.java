...
class AsyncCouponIssueServiceV1Test extends TestConfig {
  ...
  public void issue_6() throws Exception {
    //given
    long userId = 1;
    Coupon coupon = Coupon.builder()
        .type(CouponType.FIRST_COME_FIRST_SERVED)
        .title("선착순 테스트 쿠폰")
        .totalQuantity(10)
        .issuedQuantity(0).dateIssueStart(LocalDateTime.now().minusDays(1))
        .dateIssueEnd(LocalDateTime.now().plusDays(1))
        .build();
    couponJpaRepository.save(coupon);
    ...
    Assertions.assertEquals(savedIssueRequest, new ObjectMapper().writeValueAsString(request));
  }
}
